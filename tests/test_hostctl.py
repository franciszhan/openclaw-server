...
class HostControllerTests(unittest.TestCase):
    ...
    def test_owner_onboarding_prompt_uses_native_openclaw_hook(self) -> None:
        ...
        self.assertEqual(state["question_sequence"][1], "workflow_context")
        ...
        self.assertIn("message:sent", hook_metadata)
        self.assertIn('event.action !== "received"', hook_handler)
        self.assertIn('event.action === "sent"', hook_handler)
        self.assertIn("current_question_id", hook_handler)
        self.assertIn("last_hook_transition", hook_handler)
        self.assertIn('delivery_owner: "agent"', hook_handler)
        self.assertIn("FOLLOWUP_FALLBACK_DELAY_MS", hook_handler)
        self.assertIn("fallback_sent_at", hook_handler)
        self.assertIn("/usr/bin/openclaw", hook_handler)
        self.assertIn("questionFor(state, expectedQuestionId)", hook_handler)
        self.assertIn("owner_onboarding_hook_updated", script)
        self.assertIn("openclaw\", \"hooks\", \"enable", script)
        self.assertIn("already_enabled", script)
        self.assertIn('hook_enable_result.get("changed")', script)
